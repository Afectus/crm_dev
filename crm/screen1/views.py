...
class screenhome1(ListView):
	template_name = 'screenhome1.html'
	model = tax
	#paginate_by = 4
	
	def dispatch(self, request, *args, **kwargs):
		return super(screenhome1, self).dispatch(request, *args, **kwargs)
		
	def get_queryset(self):
		data=super(screenhome1, self).get_queryset()
		self.data=data.filter(status=True).order_by('id')
		return self.data
		
	def get_context_data(self, *args, **kwargs):
		context_data = super(screenhome1, self).get_context_data(*args, **kwargs)
		#context_data.update({'leftmenu': tax.objects.all(),})
		return context_data
		
class screenlist1(ListView):
	template_name = 'screenlist1.html'
	model = goods
	#paginate_by = 6
	
	def dispatch(self, request, *args, **kwargs):
		self.tax = get_object_or_404(tax, id=self.kwargs['pk'])
		return super(screenlist1, self).dispatch(request, *args, **kwargs)
		
	def get_queryset(self):
		self.data=super(screenlist1, self).get_queryset()
	...
	def get_context_data(self, *args, **kwargs):
		context_data = super(screenlist1, self).get_context_data(*args, **kwargs)
		context_data.update({'leftmenu': tax.objects.filter(status=True).order_by('id'),})
		context_data.update({'tax': self.tax,})
		return context_data
		
class Form_filter_screenlistsearch1(forms.Form):
...
class screenlistsearch1(ListView):
	...
	def dispatch(self, request, *args, **kwargs):
		#self.tax = get_object_or_404(tax, id=self.kwargs['pk'])
		return super(screenlistsearch1, self).dispatch(request, *args, **kwargs)
		
	def get_queryset(self):
	
		f=Form_filter_screenlistsearch1(self.request.GET)
		fdata = f.cleaned_data
	
		req = ''
	
		self.data=super(screenlistsearch1, self).get_queryset()
	...
	def get_context_data(self, *args, **kwargs):
		context_data = super(screenlistsearch1, self).get_context_data(*args, **kwargs)
		context_data.update({'form': Form_filter_screenlistsearch1(self.request.GET),})
...
@csrf_exempt
def screenshowvideocount1(request, pk):
