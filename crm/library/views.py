...
def download_book(request, pk): 
	...
	data = librarybookdownload.objects.create(librarybook=book, user=request.user)
	data.save()
	return HttpResponseRedirect(book.sourcefile.url)

class library_download_list(ListView):
	template_name = 'library_download_list.html'
	model = librarybookdownload
	# paginate_by = 60
	
	def dispatch(self, request, *args, **kwargs):
		get_object_or_denied(self.request.user, 'library_download_list', 'L')
		return super(library_download_list, self).dispatch(request, *args, **kwargs)
		
	# def get_queryset(self):
	# 	data=super(library_download_list, self).get_queryset()
	# 	self.data = data #for get_context_data
	# 	#data = data.filter(Q(user=self.request.user, status='close') | Q(type='free', status='close') | Q(executor__user=self.request.user, status='close')).distinct()
	# 	return data
		
	# def get_context_data(self, *args, **kwargs):
	# 	context_data = super(library_download_list, self).get_context_data(*args, **kwargs)
	# 	#context_data.update({'task_create': self.data.filter(user=self.request.user, status='open'),})
	# 	return context_data
